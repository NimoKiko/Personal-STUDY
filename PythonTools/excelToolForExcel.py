            ...
            if any(keyword in lower_name for keyword in target_keywords_list):
                ...
                try:
                    if ext == '.csv':
                        ...
                        all_data.append(df)
                        print(
                            f"已加载CSV: {filepath} ({len(df)}行, 编码: {encoding if 'encoding' in locals() else 'utf-8/gbk'})")
                    else:
                        # 处理Excel文件
                        ...
                        for sheet_name in xls.sheet_names:
                            if exclude_sheet_keyword in sheet_name.lower():
                                print(
                                    f"跳过 {filename} 中的工作表: {sheet_name} (包含{exclude_sheet_keyword}字样)")
                                continue
                            ...
                            all_data.append(df)
                            print(
                                f"已加载Excel: {filepath} (工作表: {sheet_name}, {len(df)}行)")
